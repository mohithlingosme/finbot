"""
Hanging Man candlestick strategy implementation.

Detects hanging man patterns: bearish reversal signal similar to hammer but in uptrend.
Entry short after confirmation close below pattern; stop above high.
"""
...
class HangingManStrategy(BaseStrategy):
    ...
    Detects hanging man patterns: hammer-like in uptrend, signals potential reversal.
    Small body at top with long lower shadow, bearish when in uptrend.
    """

    def __init__(self, name: str, config: Dict[str, Any]):
        """
        Initialize hanging man strategy.
        ...
        self.shadow_ratio = config.get('shadow_ratio', 2.0)  # Min lower shadow ratio to body
    ...
    def on_data(self, symbol: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Check for hanging man patterns
            self._check_hanging_man_patterns(symbol, current_data, current_atr, current_position, data)
    ...
    def _check_hanging_man_patterns(self, symbol: str, data: pd.DataFrame,
                                  atr: float, position: float, market_data: Dict[str, Any]) -> None:
        """
        Check for hanging man patterns and generate signals.
        ...
        hanging_man_info = self._detect_hanging_man(curr_candle, market_data)

        if not hanging_man_info:
            return

        # Check trend context (must be in uptrend for hanging man)
        trend_context = self._get_trend_context(data, self.lookback)

        # Hanging man only works in uptrends
        if trend_context == 'uptrend' and position >= 0:  # Allow if flat or long
            # Wait for bearish confirmation (next candle closes below hanging man low)
            if self._check_confirmation(data, 'bearish'):
                self._execute_sell_short_order(symbol, curr_candle['close'], atr, hanging_man_info['strength'], market_data)

    def _detect_hanging_man(self, candle: pd.Series, market_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Detect hanging man pattern in candle.

        Hanging Man: same structure as hammer but appears in uptrend.
        ...
        if lower_shadow < body_size * self.shadow_ratio:
            return None

        # 3. Little to no upper shadow (pattern at top)
        if upper_shadow > body_size * 0.5:  # Allow small upper shadow
            return None

        # 4. Body near top of range
        body_position = (body_high - low_price) / total_range
        if body_position < 0.7:  # Body should be in upper 30% of range
            return None
    ...
    def _check_confirmation(self, data: pd.DataFrame, direction: str) -> bool:
        """
        Check for confirmation candle after hanging man.
        """
        if len(data) < 2:
            return False

        hanging_man_candle = data.iloc[-2]  # Hanging man candle
        conf_candle = data.iloc[-1]  # Confirmation candle

        if direction == 'bearish':
            # Bearish confirmation: close below hanging man low
            return conf_candle['close'] < hanging_man_candle['low']
    ...
    def _execute_sell_short_order(self, symbol: str, price: float, atr: float,
        ...
        stop_price = market_data['high'] + (atr * 0.5)
        ...
        if position_size > 0:
            ...
            if self.submit_order(order):
                print(f"HANGING MAN SHORT order submitted: {symbol} qty={position_size:.2f} price={price:.2f}")
                notes = f"Hanging man pattern: Short {price:.2f}, Stop {stop_price:.2f}"
    ...
    def calculate_signals(self, data: pd.DataFrame, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(self.lookback, len(data)):
                ...
                # Detect hanging man
                hanging_man_info = self._detect_hanging_man(candle, market_data_at_i)
                ...
                if hanging_man_info:
                    ...
                    if confirmation and trend_context == 'uptrend':
                        stop_price = candle['high'] + (current_atr * 0.5)
                        signals.append({
                            'timestamp': data.index[i],
                            'symbol': symbol,
                            'signal_type': 'sell_short',
                            'strength': hanging_man_info['strength'],
                            'price': candle['close'],
                            'pattern': 'hanging_man',
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        ...
        return {
            'name': self.name,
            'type': 'Hanging Man Candlestick',
            'description': 'Detects hanging man patterns (hammer in uptrend) with trend and confirmation filters',
