"""
Bullish flag strategy implementation.

Detects bullish flag patterns: sharp run-up (flagpole) followed by a small downward-sloping channel (flag).
Entry on breakout above flag; stop below flag low; target ≈ flagpole height.
"""
...
class BullishFlagStrategy(BaseStrategy):
    """
    Bullish flag strategy.

    Identifies flagpole (sharp run-up) followed by consolidation channel.
    Enters on breakout above the channel with volume confirmation.
    """

    def __init__(self, name: str, config: Dict[str, Any]):
        """
        Initialize bullish flag strategy.
        ...
        # Strategy parameters
        self.flagpole_min_pct = config.get('flagpole_min_pct', 5.0)  # Minimum flagpole % increase
        self.flagpole_max_pct = config.get('flagpole_max_pct', 20.0)  # Maximum flagpole % increase
        self.channel_lookback = config.get('channel_lookback', 10)  # Bars to look for channel
        self.channel_slope_threshold = config.get('channel_slope_threshold', -0.1)  # Max downward slope
    ...
    def _check_flag_pattern_and_breakout(self, symbol: str, data: pd.DataFrame,
        ...
        """
        Check for bullish flag pattern and breakout signals.
        ...
        # Detect flag pattern
        flag_info = self._detect_bullish_flag(data)
    ...
    def _detect_bullish_flag(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        """
        Detect bullish flag pattern in recent data.
        ...
        # Look for flagpole: recent sharp run-up
        recent_highs = data['high'].tail(20)
        flagpole_start_idx = recent_highs.idxmin()  # Start of run-up
        flagpole_end_idx = len(data) - 1

        if flagpole_end_idx - flagpole_start_idx < 5:
            return None

        flagpole_start_price = data.loc[flagpole_start_idx, 'low']
        flagpole_end_price = data.loc[flagpole_end_idx, 'high']
        flagpole_pct = (flagpole_end_price - flagpole_start_price) / flagpole_start_price * 100
        ...
        # Channel should be slightly downward sloping
    ...
    def _execute_buy_order(self, symbol: str, price: float, stop_price: float,
        ...
        if position_size > 0:
            ...
            if self.submit_order(order):
                print(f"BULLISH FLAG BUY order submitted: {symbol} qty={position_size:.2f} price={price:.2f}")
                notes = f"Bullish flag breakout: Entry {price:.2f}, Stop {stop_price:.2f}, Target {target_price:.2f}"
    ...
    def calculate_signals(self, data: pd.DataFrame, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(self.channel_lookback + 10, len(data)):
                ...
                # Detect flag pattern
                flag_info = self._detect_bullish_flag(window_data)
                ...
                if flag_info:
                    ...
                    # Check breakout
                    if current_price > flag_info['flag_high']:
                        # Volume confirmation
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        ...
        return {
            'name': self.name,
            'type': 'Bullish Flag',
            'description': 'Detects bullish flag patterns and enters on breakouts with volume confirmation',
