"""
Bullish pennant strategy implementation.

Detects bullish pennant patterns: small symmetrical triangle after strong move.
Entry on breakout continuation in pole direction; stop inside pennant; target ≈ pole height.
"""
...
class BullishPennantStrategy(BaseStrategy):
    """
    Bullish pennant strategy.

    Identifies pennant consolidation after strong uptrend and enters on breakout.
    """

    def __init__(self, name: str, config: Dict[str, Any]):
        """
        Initialize bullish pennant strategy.
        ...
        # Strategy parameters
        self.pole_min_pct = config.get('pole_min_pct', 5.0)  # Minimum pole % increase
        self.pole_max_pct = config.get('pole_max_pct', 20.0)  # Maximum pole % increase
    ...
    def _check_pennant_and_breakout(self, symbol: str, data: pd.DataFrame,
        ...
        """
        Check for bullish pennant pattern and breakout signals.
        ...
        pennant_info = self._detect_bullish_pennant(data)
        ...
        if current_price > pennant_high and position == 0:
    ...
    def _detect_bullish_pennant(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        """
        Detect bullish pennant pattern in recent data.
        ...
        # Look for pole: recent sharp run-up
        recent_highs = data['high'].tail(20)
        pole_start_idx = recent_highs.idxmin()
        pole_end_idx = len(data) - 1

        if pole_end_idx - pole_start_idx < 5:
            return None

        pole_start_price = data.loc[pole_start_idx, 'low']
        pole_end_price = data.loc[pole_end_idx, 'high']
        pole_pct = (pole_end_price - pole_start_price) / pole_start_price * 100
    ...
    def _execute_buy_order(self, symbol: str, price: float, stop_price: float,
        ...
        if position_size > 0:
            ...
            if self.submit_order(order):
                print(f"BULLISH PENNANT BUY order submitted: {symbol} qty={position_size:.2f} price={price:.2f}")
                notes = f"Bullish pennant breakout: Entry {price:.2f}, Stop {stop_price:.2f}, Target {target_price:.2f}"
    ...
    def calculate_signals(self, data: pd.DataFrame, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(self.pennant_lookback + 10, len(data)):
                ...
                # Detect pennant pattern
                pennant_info = self._detect_bullish_pennant(window_data)

                if pennant_info:
                    current_price = data.iloc[i]['close']

                    # Check breakout
                    if current_price > pennant_info['pennant_high']:
                        signals.append({
                            ...
                            'pennant_low': pennant_info['pennant_low'],
                            'pole_height': pennant_info['pole_height'],
                            'stop_price': pennant_info['pennant_low'],
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'description': 'Detects bullish pennant patterns and enters on breakouts with volume confirmation',
