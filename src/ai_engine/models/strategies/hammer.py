"""
Hammer candlestick strategy implementation.

Detects hammer patterns: small body at top with long lower shadow — rejection of lows.
Entry buy after confirmation close above hammer; stop below shadow.
"""
...
class HammerStrategy(BaseStrategy):
    ...
    Detects hammer patterns with trend and volume confirmation.
    Hammers signal potential bullish reversals at support levels.
    """

    def __init__(self, name: str, config: Dict[str, Any]):
        """
        Initialize hammer strategy.
    ...
    def on_data(self, symbol: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._check_hammer_patterns(symbol, current_data, current_atr, current_position, data)
    ...
    def _check_hammer_patterns(self, symbol: str, data: pd.DataFrame,
                             atr: float, position: float, market_data: Dict[str, Any]) -> None:
        """
        Check for hammer patterns and generate signals.
        ...
        hammer_info = self._detect_hammer(curr_candle, market_data)

        if not hammer_info:
            return

        # Check trend context (should be in downtrend or at support)
        trend_context = self._get_trend_context(data, self.lookback)

        # Hammer works best in downtrends or sideways markets
        if trend_context in ['downtrend', 'sideways'] and position == 0:
            # Wait for bullish confirmation (next candle closes above hammer high)
            if self._check_confirmation(data, 'bullish'):
                self._execute_buy_order(symbol, curr_candle['close'], atr, hammer_info['strength'], market_data)

    def _detect_hammer(self, candle: pd.Series, market_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Detect hammer pattern in candle.

        Hammer: small body near top, long lower shadow, little/no upper shadow.
        """
        ...
        # 3. Little to no upper shadow (hammer at top)
    ...
    def _check_confirmation(self, data: pd.DataFrame, direction: str) -> bool:
        """
        Check for confirmation candle after hammer.
        """
        if len(data) < 2:
            return False

        hammer_candle = data.iloc[-2]  # Hammer candle
        conf_candle = data.iloc[-1]   # Confirmation candle

        if direction == 'bullish':
            # Bullish confirmation: close above hammer high
            return conf_candle['close'] > hammer_candle['high']
    ...
    def _execute_buy_order(self, symbol: str, price: float, atr: float,
        ...
        if position_size > 0:
            ...
            if self.submit_order(order):
                print(f"HAMMER BUY order submitted: {symbol} qty={position_size:.2f} price={price:.2f}")
                notes = f"Hammer pattern: Entry {price:.2f}, Stop {stop_price:.2f}"
    ...
    def calculate_signals(self, data: pd.DataFrame, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(self.lookback, len(data)):
                ...
                # Detect hammer
                hammer_info = self._detect_hammer(candle, market_data_at_i)
                ...
                if hammer_info:
                    ...
                    if confirmation and trend_context in ['downtrend', 'sideways']:
                        stop_price = candle['low'] - (current_atr * 0.5)
                        signals.append({
                            'timestamp': data.index[i],
                            'symbol': symbol,
                            'signal_type': 'buy',
                            'strength': hammer_info['strength'],
                            'price': candle['close'],
                            'pattern': 'hammer',
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        ...
        return {
            'name': self.name,
            'type': 'Hammer Candlestick',
            'description': 'Detects hammer patterns with trend and confirmation filters',
