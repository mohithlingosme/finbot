"""
Shooting Star candlestick strategy implementation.

Detects shooting star patterns: small body at bottom with long upper shadow — rejection of highs.
Entry short after confirmation close below star; stop above shadow.
"""
...
class ShootingStarStrategy(BaseStrategy):
    ...
    Detects shooting star patterns with trend and volume confirmation.
    Shooting stars signal potential bearish reversals at resistance levels.
    """

    def __init__(self, name: str, config: Dict[str, Any]):
        """
        Initialize shooting star strategy.
    ...
    def on_data(self, symbol: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._check_shooting_star_patterns(symbol, current_data, current_atr, current_position, data)
    ...
    def _check_shooting_star_patterns(self, symbol: str, data: pd.DataFrame,
                                    atr: float, position: float, market_data: Dict[str, Any]) -> None:
        """
        Check for shooting star patterns and generate signals.
        ...
        star_info = self._detect_shooting_star(curr_candle, market_data)

        if not star_info:
            return

        # Check trend context (should be in uptrend or at resistance)
        trend_context = self._get_trend_context(data, self.lookback)

        # Shooting star works best in uptrends or sideways markets
        if trend_context in ['uptrend', 'sideways'] and position >= 0:  # Allow if flat or long
            # Wait for bearish confirmation (next candle closes below star low)
            if self._check_confirmation(data, 'bearish'):
                self._execute_sell_short_order(symbol, curr_candle['close'], atr, star_info['strength'], market_data)

    def _detect_shooting_star(self, candle: pd.Series, market_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Detect shooting star pattern in candle.

        Shooting Star: small body near bottom, long upper shadow, little/no lower shadow.
        """
        ...
        # 3. Little to no lower shadow (star at bottom)
    ...
    def _check_confirmation(self, data: pd.DataFrame, direction: str) -> bool:
        """
        Check for confirmation candle after shooting star.
        """
        if len(data) < 2:
            return False

        star_candle = data.iloc[-2]  # Shooting star candle
        conf_candle = data.iloc[-1]  # Confirmation candle

        if direction == 'bearish':
            # Bearish confirmation: close below star low
            return conf_candle['close'] < star_candle['low']
    ...
    def _execute_sell_short_order(self, symbol: str, price: float, atr: float,
        ...
        if position_size > 0:
            ...
            if self.submit_order(order):
                print(f"SHOOTING STAR SHORT order submitted: {symbol} qty={position_size:.2f} price={price:.2f}")
                notes = f"Shooting star pattern: Short {price:.2f}, Stop {stop_price:.2f}"
    ...
    def calculate_signals(self, data: pd.DataFrame, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(self.lookback, len(data)):
                ...
                star_info = self._detect_shooting_star(candle, market_data_at_i)
                ...
                if star_info:
                    ...
                    if confirmation and trend_context in ['uptrend', 'sideways']:
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        ...
        return {
            'name': self.name,
            'type': 'Shooting Star Candlestick',
            'description': 'Detects shooting star patterns with trend and confirmation filters',
